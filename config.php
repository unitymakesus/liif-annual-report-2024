<?php

return [
    'title'       => 'LIIF 2024 Annual Impact Report',
    'description' => 'Investing in the future. 40 years strong.',
    'staging'     => false,
    'baseUrl'     => 'https://liif-annual-report-2024.test',
    'social'      => [
        'linkedin' => 'https://www.linkedin.com/company/low-income-investment-fund/',
        'facebook' => 'https://www.facebook.com/liifund/',
        'x'        => 'https://x.com/Liifund',
        'youtube'  => 'https://www.youtube.com/user/LIIFund',
    ],
    'supporters' => [
        'Amalgamated Bank',
        'Ballmer Group',
        'California Pollution Control Financing Authority',
        'Capital One Foundation',
        'Cathay Bank Foundation',
        'CDFI Fund - Assistance Awards',
        'CDFI Fund - Capital Magnet Fund Program',
        'CDFI Fund - Equitable Recovery Program',
        'CDFI Fund - New Markets Tax Credit Program',
        'Charles Schwab Bank, SSB',
        'Child Care Aware of America',
        'D.C. Department of Housing and Community Development',
        'Deutsche Bank Trust Company Americas',
        'Federal Home Loan Bank of San Francisco',
        'First Citizens Bank and Trust Company',
        'Georgia Power Foundation',
        'Goldman Sachs Foundation',
        'Harris County Department of Economic Equity and Opportunity',
        'Heising-Simons Foundation',
        'JPMorgan Chase Bank, N.A.',
        'Local Initiatives Support Corporation (LISC)',
        'Marisla Fund',
        'Mimi and Peter Haas Fund',
        'Morgan Stanley',
        'New York Life Foundation',
        'Office of the State Superintendent of Education (OSSE)',
        'Opportunity Finance Network',
        'Purpose Built Communities Foundation, Inc.',
        'QUALITYstarsNY',
        'Robert Wood Johnson Foundation',
        'Santander Bank, N.A.',
        'Silicon Valley Community Foundation',
        'The Allstate Foundation',
        'The Alphadyne Foundation',
        'The Annie E. Casey Foundation, Inc.',
        'The Community Foundation for Greater Atlanta, Inc.',
        'U.S. Bank Foundation',
        'U.S. Department of Education',
        'Wells Fargo and Company',
        'Wells Fargo Foundation',
        'Western Alliance Community Foundation',
    ],
    'officers' => [
        [
            'name'  => 'Reymundo Ocañas, Chair',
            'title' => 'Executive Vice President, Director of Community Development Banking, Corporate Responsibility Group, PNC Bank',
        ],
        [
            'name'  => 'Jessica Sager, Vice Chair',
            'title' => 'CEO, All Our Kin, Inc.',
        ],
        [
            'name'  => 'Russell J. Bruemmer, Secretary',
            'title' => 'Retired Partner, Wilmer Cutler Pickering Hale and Dorr',
        ],
        [
            'name'  => 'Eileen Fitzgerald, Treasurer',
            'title' => 'Founder and Principal, ThruSight, LLC',
        ],
    ],
    'directors' => [
        [
            'name'  => 'Margaret Anadu',
            'title' => 'Senior Partner, The Vistria Group',
        ],
        [
            'name'  => 'Tawanna A. Black',
            'title' => 'Founder and CEO, Center for Economic Inclusion',
        ],
        [
            'name'  => 'David Fleming, M.D.',
            'title' => 'Distinguished Fellow, Trust for America’s Health',
        ],
        [
            'name'  => 'Calvin Gladney, LEED AP',
            'title' => 'President and CEO, Smart Growth America',
        ],
        [
            'name'  => 'William Kelly Jr.',
            'title' => 'Founder, Stewards of Affordable Housing for the Future',
        ],
        [
            'name'  => 'Gloria Lee',
            'title' => 'Co-Founder and Head, Rise Community Capital LLC',
        ],
        [
            'name'  => 'Carol R. Naughton',
            'title' => 'CEO, Purpose Built Communities',
        ],
        [
            'name'  => 'Dionne Nelson',
            'title' => 'President and CEO, Laurel Street',
        ],
        [
            'name'  => 'Daniel A. Nissenbaum',
            'title' => 'Chief Executive Officer, Low Income Investment Fund',
        ],
        [
            'name'  => 'Erika Poethig',
            'title' => 'Executive Vice President for Strategy and Planning, Civic Committee and Commercial Club of Chicago; Member of the Illinois Housing Development Authority',
        ],
        [
            'name'  => 'Yohana Quiróz, Ed.D.',
            'title' => 'Chief Operations Officer, Felton Institute',
        ],
        [
            'name'  => 'Michael A. Solomon',
            'title' => 'Managing Director, Community Development, Charles Schwab Banking and Trust Services',
        ],
    ],
    'senior_staff' => [
        [
            'name'  => 'Daniel A. Nissenbaum',
            'title' => 'Chief Executive Officer',
        ],
        [
            'name'  => 'Kimberly Latimer-Nelligan',
            'title' => 'President',
        ],
        [
            'name'  => 'Panagiota Mahendru',
            'title' => 'Executive Vice President and Chief Financial & Administrative Officer',
        ],
        [
            'name'  => 'Denise Noel',
            'title' => 'Executive Vice President and General Counsel',
        ],
        [
            'name'  => 'Dr. Tina Walker',
            'title' => 'Executive Vice President and Chief People Officer',
        ],
        [
            'name'  => 'Maygen Moore',
            'title' => 'Interim Chief Lending Officer',
        ],
        [
            'name'  => 'Rachel Bluestein',
            'title' => 'Senior Vice President and Chief Program Officer',
        ],
        [
            'name'  => 'Angie Garling',
            'title' => 'Senior Vice President, National ECE',
        ],
        [
            'name'  => 'Susan Hyman',
            'title' => 'Senior Vice President and Chief Credit Officer',
        ],
        [
            'name'  => 'Amir Ali',
            'title' => 'Vice President for National Community Facilities Lending and New Markets Tax Credits',
        ],
        [
            'name'  => 'Pam Berkowitz',
            'title' => 'Vice President, Western Region',
        ],
        [
            'name'  => 'Walter Chen',
            'title' => 'Vice President and Associate General Counsel',
        ],
        [
            'name'  => 'Kim DiGiacomo',
            'title' => 'Vice President, ECE Capacity Building',
        ],
        [
            'name'  => 'LaToya Kyle',
            'title' => 'Vice President, Southeast Region',
        ],
        [
            'name'  => 'Mona Malan',
            'title' => 'Vice President, ECE Facilities Funds Management',
        ],
        [
            'name'  => 'Shelly Masur',
            'title' => 'Vice President, ECE Advisory and State Policy',
        ],
        [
            'name'  => 'Tiffany Mobley',
            'title' => 'Vice President, ECE Government Contracts and Operations',
        ],
        [
            'name'  => 'Kirsten Shaw',
            'title' => 'Vice President, Northeast and Mid-Atlantic Regions',
        ],
        [
            'name'  => 'Christina Shiwbalak',
            'title' => 'Vice President of Operations',
        ],
        [
            'name'  => 'Jessica Standiford',
            'title' => 'Vice President, Development and Impact Investing',
        ],
        [
            'name'  => 'Paul Wetterholm',
            'title' => 'Interim Controller',
        ],
    ],
];
